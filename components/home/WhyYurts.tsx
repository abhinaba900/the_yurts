...
const featureIcons = [
  ...
  // 02 Designed for different ground
  (
    <svg aria-hidden="true" className="size-5 text-accent-text opacity-70" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
 ...
 * 04. WHY YURTS
 *
 * Section label: WHY A YURT
 * Heading: Build less. Experience more.
 */
  ...
  return (
    <section className="u-container py-16 lg:py-20 lg:min-h-[calc(100vh-6rem)] lg:flex lg:flex-col lg:justify-center overflow-hidden">
      ...
      <div className="flex flex-wrap items-end justify-between gap-x-12 gap-y-4 border-b border-line pb-6 lg:pb-8">
        <div>
          <Reveal kind="up">
            <Metadata className="text-accent-text">Why a Yurt</Metadata>
            <h2 className="mt-2 font-display text-display-lg u-optical-left">
              Build less. Experience more.
            </h2>
          </Reveal>
        </div>
        <Reveal kind="up" delay={0.08} className="max-w-md">
          <p className="font-sans text-small text-text-muted">
            Four fundamental advantages over conventional permanent construction.
          </p>
        </Reveal>
      </div>

      {/* 2x2 Prominent Editorial Pillars Grid */}
      <div className="mt-8 lg:mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:gap-6">
        {whyYurts.map((item, i) => (
          <Reveal
            key={item.title}
            kind="up"
            delay={i * 0.04}
            className="group relative flex flex-col justify-between overflow-hidden rounded-sm border border-line/70 bg-surface-alt/40 p-6 lg:p-8 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-accent/40 hover:bg-surface-alt/70 hover:shadow-xl"
          >
            <div>
              ...
              <p className="mt-3 font-sans text-body text-text-muted leading-relaxed">
                {item.body}
              </p>
            </div>
      ...
      {/* Footer Navigation */}
      <div className="mt-8 border-t border-line pt-5 flex flex-wrap items-center justify-between gap-4">
        <ArrowLink href="/why-theyurts">
          Discover the complete architectural case
        </ArrowLink>
